...
% grid intervals
nGrid = 50; % nGrid +1 nodes
nNodes = nGrid+1;
var = 9;

  t0s = '2029-02-08-00:00:00.000 UTC';
  tFs = '2029-02-15-23:59:59.000 UTC';
...
stateStart = [5;5;5;1e-12;1e-12;1e-12]; % [km]
stateTarget = [15;-50; -15; 1.5e-12; 1.5e-12;1.5e-12]; % [km, km/s]
x0(1:6,1) = stateStart;
x0(end-9:end-4,1) = stateTarget;
...
[xMin,fval,~, ~] = fmincon(fMinimize,x0, ... 
...
arrivalDate = cspice_et2utc(fval,'C',0)
title(['Sprint ending on ',arrivalDate])
for jj = 1:nNodes-1
%     xMin((jj-1)*var+1:(jj-1)*var+3)
   statePlot(:,jj) = xMin((jj-1)*var+1:(jj-1)*var+3);
   
   plot3(statePlot(1,jj),statePlot(2,jj),statePlot(3,jj),'*')
   hold on
end
...
title(['Sprint ending on ',arrivalDate])
hold on
plot3(0,0,0,'ok','MarkerSize',4, 'MarkerFaceColor','k')
%% FUNCTIONS
...
%         accJ2 =kJ2*(1-5*RJ2000(3)^2/nRJ2000^2)*RJ2000/nRJ2000;
    ...
    dY = [
        ...
        v;
        w;
        totAcc(1)+throttle*Tmax/m*cos(alpha)*cos(beta)*1e-3;
        totAcc(2)+throttle*Tmax/m*sin(alpha)*cos(beta)*1e-3;
        totAcc(3)+throttle*Tmax/m*sin(beta)*1e-3;
...
%         obj = 1/Y(end-3,1);
        if nargout >1
            ...
            grad = [grad;1];
%             grad = zeros(length(Y)-4,1);
%             grad = [grad;-1/(Y(end-3)^2);0;0;0];
        end
    else 
        obj = 1/Y(end-2,1);
        if nargout >1
